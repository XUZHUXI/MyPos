...
public class AppMenuV2  extends Application{

    public   TilePane   getProductCategoryMenu(     ){
        
        //取得產品清單
        TreeMap<String, Product> product_dict = ReadCategoryProduct.readProduct();
        //磁磚窗格
        TilePane menu = new TilePane(); //
        menu.setVgap(10);  //垂直間隙
        menu.setHgap(10);
    ...
    public void start(Stage stage) throws Exception {
        ...
        //取得菜單磁磚窗格並放入根容器
        VBox menuContainerPane = new VBox(); //這個容器在下一個教學中會被放到全域變數
        menuContainerPane.getChildren().add(this.getProductCategoryMenu());
        //放入根容器
